...
const CoversContent = ({ covers=coversArr }) => {
    ...
    const handleClick = (i) => {
        ...
        if(coverList[i].playing) {
            ...
            axios.get(`http://localhost:4002/covers/${i + 1}`).then((res) => {
    ...
    return (
        <div className='contentContainer bg-itm-band-photo bg-[center_10px] centerItems'>
            ...
            {coverList.map((cover, index) => {
                return <CoverLink color={cover.color} image={cover.image} onClick={() => handleClick(index)} musicPlaying={cover.playing} key={cover.name} />
            })}
        </div>
    )
}
...
const CoverLink = ({ color='bg-gray-800', image='bg-logo', onClick, musicPlaying }) => {
